...
export function getFunctions(
  ...
  if (extension === ".ts" || extension === ".js") {
    ...
    functionRegex =
      /(?:'''([\s\S]*?)'''|"""([\s\S]*?)""")\s*(async\s+)?def\s+(\w+)\s*\((.*?)\)(?:\s*->\s*(\w+))?\s*:/gs;
  } else {
  ...
  matches.forEach((match) => {
    if (extension === ".ts" || extension === ".js") {
      ...
      const returnType =
        match[6] || match[10] || match[14] || match[18] || "any";
...
export function parseJSDocstring(docstring: string) {
  ...
  const returnsMatch = docstring.match(
    /@returns?\s+{?([^}]*)}?\s+(.*?)(?=@|\n\s*\n|$)/s,
  );
  if (returnsMatch) {
  ...
  const throwsMatches = docstring.matchAll(
    /@throws?\s+{?([^}]*)}?\s+(.*?)(?=@|\n\s*\n|$)/gs,
  );
...
export function parsePythonDocstring(docstring: string) {
  ...
  const paramSection = docstring.match(
    /Args:|Parameters:?\s*([\s\S]*?)(?=Returns:|Raises:|Yields:|Examples:|$)/i,
  );
  if (paramSection) {
  ...
  const returnsSection = docstring.match(
    /Returns:|Return:?\s*([\s\S]*?)(?=Raises:|Yields:|Examples:|$)/i,
  );
  if (returnsSection) {
  ...
  const raisesSection = docstring.match(
    /Raises:|Exceptions:?\s*([\s\S]*?)(?=Returns:|Yields:|Examples:|$)/i,
  );
  if (raisesSection) {
